...
def sequence_mask(sequence_length, max_len=None):
    ...
    seq_range = torch.arange(0, max_len).long() #生成0-max_len的序列
    seq_range_expand = seq_range.unsqueeze(0).expand(batch_size, max_len)  #在第0维增加一个维度
...
def cross_entropy(logits, target):
    ...
    losses_flat = -torch.gather(log_probs_flat, dim=1, index=target) #去第几行第几个，比如某个样本分类为5，那么取
    # 该行第5个概率计算
